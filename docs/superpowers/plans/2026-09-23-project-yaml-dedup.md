...
- **The gate must pass** (`gate.py check`, Task 0) after Task 4 and Task 5 with only the allowlisted difference: the package-less `root`, `root/ppg/staging` and `root/ppg/devel` projects lose `Prefer: libverto-libev` and `Prefer: Lmod` (the `root/ppg/releases` tier loses them too but the gate skips everything under `releases/`). Never widen the allowlist to make it pass; fix the yaml instead. The gate only baselines projects with a `project.yaml`, so it covers three of the nine package-less projects that actually lose the two `Prefer` lines; the other six (`ROOT:common`, `ROOT:common:containers`, `ROOT:common:deps`, `ROOT:ppg`, `ROOT:ppg:common`, `ROOT:ppg:releases`) were verified separately (see the spec's Gate section).
- **Meta XML byte-identical for every pre-existing non-release project** (the new `ppg:staging:extras` project.yaml is the one documented exception). Path order inside a repository is semantic in OBS. Repository element order must not change either.
