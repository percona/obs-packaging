...
- Issues 1+2+data: **lean `percona-tarball-gdal` (+ matching `percona-tarball-proj`) rebuild** in `ppg:common:deps`, RL8+RL9, distinct names, tarball-only (shipped PostGIS RPMs keep EPEL GDAL). Same soname series as what PostGIS linked: GDAL 3.0.4/`libgdal.so.26` + PROJ 6.3.2/`libproj.so.15` on EL8; GDAL 3.4.3/`libgdal.so.30` + PROJ 9.x/`libproj.so.25` on EL9. Official's driver set; data dirs compiled into the /opt prefix.
...
### Task 22: `percona-tarball-gdal` + `percona-tarball-proj` (common:deps, RL8+RL9)
...
**Changes:**
- simpleimage: `BuildRequires: percona-psql percona-tarball-gdal percona-tarball-proj`. Ensure EPEL `gdal-libs`/`proj` are NOT pulled: PostGIS Requires resolve to ours via prjconf `Prefer: percona-tarball-gdal` / `Prefer: percona-tarball-proj` (verify with buildinfo after sync; if EPEL still lands in the chroot, add `Ignore:` or explicit conflicts).
