...
%if %{raster}
%if 0%{?rhel} && 0%{?rhel} >= 9
BuildRequires:	gdal3.4-devel >= %{gdalfullversion}
Requires:	gdal3.4-libs >= %{gdalfullversion}
%else
%if 0%{?rhel} && 0%{?rhel} >= 8
BuildRequires:	gdal-devel >= %{gdalfullversion}
Requires:	gdal-libs >= %{gdalfullversion}
%else
...
Suggests:	percona-postgis-tiger-geocoder
