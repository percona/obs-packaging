...
%endif
%if 0%{?suse_version} > 1600
Requires:        llvm-devel clang-devel
%endif
%if 0%{?fedora} || 0%{?rhel} >= 8
...
%endif
%if 0%{?suse_version} >= 1500 && 0%{?suse_version} < 1600
export CLANG=%{_bindir}/clang-17
%endif
%if 0%{?suse_version} == 1600
export CLANG=%{_bindir}/clang-19
%endif
# On openSUSE, llvm-config is not at a stable path (llvm19-devel provides
# /usr/lib/llvm19/bin/llvm-config, not /usr/bin/llvm-config). Let PG's
# configure locate a working llvm-config via PATH by not exporting it here.
# Tumbleweed (suse_version > 1600) uses unversioned llvm-devel/clang-devel.
...
# Install the rpm macros file.
install -D -m 0644 %{SOURCE20} %{buildroot}%{_rpmmacrodir}/macros.percona-postgresql

%if %sysuserd
# Install sysusers.d config file to allow rpm to create users/groups automatically.
%{__install} -m 0644 -D %{SOURCE21} %{buildroot}%{_sysusersdir}/%{sname}%{pgpackageversion}-pgdg.conf
%endif
...
%sysusers_create_package %{name} %SOURCE21
%else
...
%dir %{pgbaseinstdir}/share/locale/*
%dir %{pgbaseinstdir}/share/locale/*/LC_MESSAGES
%dir %{pgbaseinstdir}/share/man
%dir %{pgbaseinstdir}/share/man/man1
%dir %{pgbaseinstdir}/share/man/man3
%dir %{pgbaseinstdir}/share/man/man7
...
%dir %{pgbaseinstdir}/share/timezonesets
...
%{_rpmmacrodir}/macros.percona-postgresql
