...
│   │   └── runtime/        # runtime dependencies shared across products
...
- **`common/deps/runtime/`** — runtime packages required by product packages across all products.
  Each `staging/<major-version>/` subproject that needs these packages should aggregate them via an
  `_aggregate` file, so that the RPM or Debian repository generated by OBS is self-contained and
  consumers do not need to add a separate repository to satisfy runtime dependencies.
